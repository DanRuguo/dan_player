...
- Flutter 分别绘制 480×240 紧凑缩略图和 1280×800 Peek 源；大预览采用独立的封面、标题和状态排版，
  字体重新栅格化。同一次方法调用发送，两份都校验/分配成功后才提交。Peek 像素占 4,096,000 字节，
  在 4 MiB 上限内留有余量。主题渐变和元数据面板随主题更新；仅曲目、主题、字体、播放/缓冲状态变化时重绘，
  显示真实总时长，不订阅播放进度，不生成持续动画。
...
- `WM_DWMSENDICONICLIVEPREVIEWBITMAP` 与缩略图分开布局：普通窗口按 `GetClientRect`
  输出客户区画布，卡片以 1120×700 逻辑像素为基准，按当前 DPI 等比居中，且永不超过源分辨率。
  新源在 96 DPI 为 1120×700，较高 DPI 最大为 1280×800；四周按源的上下背景色补充渐变。
