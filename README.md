...
| **26.0.5-snapshot.3** | 正式版前的候选预览 | [签名安装器、便携 ZIP 与 SHA256SUMS](https://github.com/DanRuguo/dan_player/releases/tag/v26.0.5-snapshot.3) |
...
播放器示例已更新至 26.0.5-snapshot.3，由生产页面与控件离屏渲染，只使用虚构曲目、路径、播放记录和几何封面；不会读取用户曲库、私人歌单或桌面。完整明暗／宽窄版本与复现方法见 [28 张安全界面示例](docs/images/README.md)。
...
提供 RCEIT.Inc 签名安装器与便携包。详见 [更新说明](docs/release-26.0.5-snapshot.3.md) · [使用与验证说明](docs/26.0.5-snapshot.3-validation.md)。

## 26.0.4 更新

新增常用歌单与本地播放功能，完善联网来源和 Windows 桌面交互。

- 新增智能歌单、CUE 分轨和播放／A-B 书签，队列整理支持撤销。
- 扩展批量操作与 M3U8 导入导出，改进排序、封面和曲库刷新。
- 完善联网搜索、歌词与评论，新增网易增强 API 适配。
- 美化搜索分类、歌词按钮与进度条，修复迷你歌词和弹窗布局。
- 增加 Windows 右键快捷任务、单实例与卸载入口，改进启动和安装升级。

详见 [更新说明](docs/release-26.0.4.md) · [使用与验证说明](docs/26.0.4-validation.md)。
