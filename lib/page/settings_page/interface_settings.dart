...
enum InterfaceSettingsGroup { all, language, layout }

/// Persistence owner for UI language/layout; never restarts audio.
class InterfaceSettings extends StatefulWidget {
  const InterfaceSettings(
      {super.key, this.persist, this.group = InterfaceSettingsGroup.all});
  final InterfaceSettingsGroup group;
...
class _InterfaceSettingsState extends State<InterfaceSettings> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      if (widget.group != InterfaceSettingsGroup.layout)
        SettingsSurface(
            child: Column(
      ...
      if (widget.group == InterfaceSettingsGroup.all)
        const SizedBox(height: 16),
      if (widget.group != InterfaceSettingsGroup.language)
