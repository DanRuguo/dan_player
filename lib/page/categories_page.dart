...
class _CategoriesPageState extends State<CategoriesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = groups
        ...
        .toList();
    final order = _presentation.orders[_kind.name] ?? const <String>[];
    final ranks = {for (var i = 0; i < order.length; i++) order[i]: i};
    if (_presentation.sort != CategorySort.standard) {
      visible.sort((a, b) {
        final comparison = switch (_presentation.sort) {
          CategorySort.name => categoryDisplayTitle(a)
              .toLowerCase()
              .compareTo(categoryDisplayTitle(b).toLowerCase()),
          CategorySort.count => a.audios.length.compareTo(b.audios.length),
          CategorySort.custom => (ranks[a.persistenceKey] ?? order.length)
              .compareTo(ranks[b.persistenceKey] ?? order.length),
          CategorySort.standard => 0,
        };
        final directed = _presentation.descending &&
                _presentation.sort != CategorySort.custom
            ? -comparison
            : comparison;
        return directed != 0
            ? directed
