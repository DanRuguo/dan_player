...
| バージョン | 用途 | ダウンロード |
| --- | --- | --- |
| **26.0.6 snapshot1 · プレビュー版** | 歌詞編集と再生操作の改善を試す | [インストーラー・ポータブル ZIP・チェックサム](https://github.com/DanRuguo/dan_player/releases/tag/v26.0.6-snapshot.1) |
| **26.0.5 · 安定版** | 日常の音楽再生に。正式リリースを使いたい方へ | [インストーラー・ポータブル ZIP・チェックサム](https://github.com/DanRuguo/dan_player/releases/tag/v26.0.5) |
...
歌詞の試聴と音声の切り出しに使う FFmpeg は任意のコンポーネントです。手動ダウンロード時は [26.0.5 で公開した独立パッケージ](https://github.com/DanRuguo/dan_player/releases/tag/v26.0.5)を再利用し、固定したサイズと SHA-256 を検証します。26.0.6 snapshot1 のインストーラーとポータブル ZIP には重複して同梱しません。
...
これらの機能は 26.0.5 安定版に含まれています。配布パッケージに含まれる機能は、対応する [Release の更新内容](https://github.com/DanRuguo/dan_player/releases)をご確認ください。
...
- タップ編集でテキスト歌詞を保存する際、翻訳と読みはテキスト版に含まれないことを確認し、本文だけを保存します。行・単語の時刻付けを続ければ補助内容を保持できます。
- 従来のテキスト編集では、長い無損失歌詞の画面更新時に重複する解析と一時的な JSON の生成を減らしました。2 種類の編集画面の幅広・幅狭表示例を追加しました。
- 歌詞とコメントは標準でローカル内容とキャッシュを優先します。設定で自動通信を有効にでき、手動検索・更新・一括キャッシュも引き続き利用できます。

### 26.0.5 の新機能

- **検索履歴：** 検索欄を少し上へ移動し、重複を除いた履歴を中央揃えのチップで表示します。自動で折り返し、狭い画面では古い履歴を隠します。クリックで再検索、右クリックまたは長押しの後に × を押すと削除できます。
- **歌詞とコメント：** 標準ではローカル内容とキャッシュを優先し、自動通信は設定で有効にできます。歌詞の手動検索、コメントの更新・追加読み込み、分類・提供元の切り替えでも通信できます。更新中はアイコンが回転し、成功・失敗を共通の通知バブルで知らせます。
...
プレイリスト表示、音量パネル、メニュー、Windows のぼかし切り替え、ウィンドウサイズの復元も改善しました。タイル配置と文字色の解析は必要時だけ行い、結果をキャッシュします。タイルのアニメーション終了後は描画更新を停止します。実際の消費電力は機器と設定により異なります。
...
| --- | --- |
| ![中国語 UI のタップ編集：本文・翻訳・読みの入力](docs/images/lyric-tap-text-zh.png) | ![中国語 UI の狭いタップ編集：単語の時刻付け](docs/images/lyric-tap-words-narrow-zh.png) |

| テキスト編集：歌詞コード | テキスト編集：単語のプレビュー |
| --- | --- |
| ![中国語 UI のテキスト編集：QRC コード](docs/images/lyric-code-editor-zh.png) | ![中国語 UI のテキスト編集：単語ごとのプレビュー](docs/images/lyric-editor-preview-zh.png) |
...
**自分の評価とタグを付ける。** 26.0.5 では、分類画面の **「個人ライブラリ → 楽曲」** で、評価や個人タグを付けた曲を表示し、日付・評価・タグで絞り込めます。個人の評価とタグはプレーヤーのデータに保存され、**音楽ファイルには書き込まれません**。一方、メタデータ編集やタグの一括編集はファイルの内容を変更するため、保存前に変更のプレビューを確認してください。
