...
/// One message surface for the player, desktop lyrics, and isolated routes.
/// The messenger is only a transport fallback when no overlay host is mounted;
/// it renders the same bounded bubble instead of a full-width Material bar.
void showPresentationNotice(
  String text, {
  ...
  final liveContext = context?.mounted == true ? context : null;
  final localMessenger =
      liveContext == null ? null : ScaffoldMessenger.maybeOf(liveContext);
  final appMessenger =
      fallbackMessenger?.mounted == true ? fallbackMessenger : null;
  final messenger = localMessenger ?? appMessenger;
  // Clear the hidden normal messenger too when a compact route owns another.
  // Removed timers/queued feedback must never reappear after a newer result.
  for (final target in {localMessenger, appMessenger}) {
    target?.clearSnackBars();
    target?.removeCurrentSnackBar();
  }
  final presentation = AppPresentationHost.maybeOf(liveContext);
  ...
  final themeContext = liveContext ?? messenger.context;
  final media = MediaQuery.maybeOf(themeContext);
  if (media == null) return;
  final notice =
      _Notice(text, kind, ++_fallbackRevision, actionLabel, onAction);
  final layout = _NoticeLayout.measure(themeContext, notice, media.size);
  messenger.showSnackBar(
      SnackBar(
        key: ValueKey(('app-notice-fallback', notice.revision)),
...
class AppPresentationHost extends StatefulWidget {
  ...
  static AppPresentationHostState? maybeOf(BuildContext? context) {
    final local = context?.mounted == true
        ? context!.getInheritedWidgetOfExactType<_PresentationScope>()?.host
        : null;
    final host = local ?? _current;
    return host?.mounted == true ? host : null;
  }
...
class AppPresentationHostState extends State<AppPresentationHost> {
  ...
  void showNotice(
    ...
    void present() {
      ...
      _changed();
      // Accessible actions must remain available until explicitly dismissed.
      if (actionLabel == null ||
          onAction == null ||
  ...
  void dismissNotice() {
    ...
    _closing = true;
    _changed();
    _removal = Timer(AppMotion.quick, () {
      if (!mounted || revision != _revision) return;
      _notice = null;
      _changed();
    });
...
Future<T?> showAppDialog<T>({
  ...
  // Match an explicit Dialog.insetPadding override in the builder. The host's
  // normal theme uses 12px below; that margin already separates the surface
  // from a notice and must not be counted twice in the reserved strip.
  double dialogBottomInset = 12,
}) {
  assert(dialogBottomInset >= 0);
  ...
  return navigator.push<T>(_CompletingDialogRoute<T>(
    ...
    builder: (dialogContext) => host == null
        ...
        : _DialogRegionScope(
            region: region,
            child: _AppDialogFrame(
                host: host,
                region: region,
                builder: builder,
                dialogBottomInset: dialogBottomInset),
          ),
  ));
}
...
class _AppDialogFrame extends StatelessWidget {
  const _AppDialogFrame(
      ...
      required this.dialogBottomInset});
  final AppPresentationHostState host;
  final _ContentRegionHandle? region;
  final WidgetBuilder builder;
  final double dialogBottomInset;
  @override
  Widget build(BuildContext context) {
    UiLanguageScope.watch(context);
    return AnimatedBuilder(
      ...
      builder: (context, dialogChild) =>
        ...
        final reserve = notice == null
            ...
            : math.max(
                0.0,
                _NoticeLayout.measure(context, notice, rect.size)
                        .reservedHeight -
                    dialogBottomInset);
              ...
              child: MediaQuery(
                ...
                child: Theme(
                  data: theme.copyWith(
                    ...
                    alignment: _NoticeSafeAlignment(reserve),
...
/// The reserved strip reduces the dialog's available height. Compensate its
/// center by half that strip when there is room, clamping at the safe bottom.
/// Alignment receives the actual post-layout free space, so this also handles
/// intrinsic AlertDialogs and scrollable editors without measuring/rebuilding
/// their contents or permanently reserving space when no notice is present.
class _NoticeSafeAlignment extends Alignment {
  const _NoticeSafeAlignment(this.reserve) : super(0, 0);

  final double reserve;

  @override
  Offset alongOffset(Offset other) => Offset(
        other.dx / 2,
        other.dy / 2 + math.min(reserve / 2, math.max(0, other.dy) / 2),
      );

  // RenderAlign must relayout when a notice arrives or leaves even though the
  // nominal x/y remain centered. Alignment's default equality only checks x/y.
  @override
  bool operator ==(Object other) =>
      other is _NoticeSafeAlignment && other.reserve == reserve;

  @override
  int get hashCode => Object.hash(_NoticeSafeAlignment, reserve);
}

class _Notice {
  _Notice(this.text, this.kind, this.revision, this.actionLabel, this.onAction);
  final String text;
  ...
  void invokeAction(VoidCallback close) {
    if (_actionInvoked) return;
    _actionInvoked = true;
    close();
    onAction?.call();
  }
              ...
              if (layout.actionWidth > 0)
                SizedBox(
                    ...
                    child: TextButton(
                      ...
                      onPressed: () => notice.invokeAction(onClose),
